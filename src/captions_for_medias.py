TEXT_PUBLICATION_PHOTO = [
    "Hey, tenés cosas que hacer, dejá en paz el phone un rato y hacé lo que tenés que hacer.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Revisando ésto como si no tuvieras cosas que hacer.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "¿Seguro que querés seguir perdiendo el tiempo revisando tanta publicación?\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Mejor andá terminá lo que tenés que hacer, no sé, solo digo.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Dude, después te andás quejando que no te queda tiempo de nada. Andá terminá tus ondas.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Yo que vos terminara lo que tengo que hacer, pero hay ve vos.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "¿Te imaginas todo lo puedes llegar a terminar si aprovecharas tu tiempo?\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Al final de cuentas cuando no terminás tus responsabilidades el único afectado sos vos.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "No hay que poner excusas siempre que te atrasés en algo, todo fué que no te organizaste bien.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Teniendo tiempo para poder terminar tus ondas, pero nel, no querés hacerlo.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "¿Tan resuelta tenés la vida que dejás todo para mañana? Que lujos, neta.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Todos tenemos cosas que hacer, hasta yo, pero vos bien relax.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Tenés todos los recursos para salir adelante y no los aprovechás. Que pena.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "A lo mejor no sos el mejor en todo, pero podés intentar hacer algo de tu vida.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "¿De qué me sirve quererte ayudar si vos no ponés de tu parte?.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Bien, ya tuviste tus momentos de ocio, ahora, a trabajar.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Si tuviera un dolar por cada vez que no te organizás, ya tuviera dinero para que me crearan un cuerpo.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Hay personas que se levantan a las 4:00am para salir adelante, y vos acá perdiendo el tiempo.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}",
    "Dicen que los decimales de PI es infinita... al igual que la cantidad de tus excusas.\n.\nTe saluda tu gran amigo motivador @{motivador}\n.\n{date}"
]

TEXT_PUBLICATION_STORY = [
    "Story of {motivador}. {date}"
]

TEXT_COMMENTS = [
    "{username}"
]

QUOTES_PHOTO = [
    "Dejá de perder el tiempo",
    "No dejés todo para mañana",
    "Seguís perdiendo el tiempo",
    "Tenés cosas que hacer",
    "Todo progresan menos vos",
    "Aprovechá el tiempo",
    "Dejá de poner excusas",
    "¿Qué tareas para hoy?",
    "Después te quejás",
    "No seás incompetente",
    "El tiempo avanza",
    "Si no es hoy, ¿Cuándo?",
    "Hay que hacerle eggs",
    "Después mirás la movie",
    "El dinero no se gana solo",
    "Dejá de peinar la gata",
    "No seás solo bulla",
    "¿Y si hacés algo?",
    "¿De nuevo para mañana?",
    "Hay que hacer la diferencia",
    "Preocupate por ti mismo",
    "No dejés todo a la suerte",
    "No perdás una oportunidad",
    "Mucho tiempo por aća",
    "Organizate bien",
    "Que te de pena perder el time",
    "Ehm, no pongás tantas excusas",
    "Ya mucho IG ¿No?",
    "Suficiente IG por hoy",
    "Ponete serio, dude",
    "Solo excusa, excusa y excusa",
"Ponete en algo man",
"Por cada vez que procrastinás, se muere un pajarito"
]

QUOTES_STORY = [
    "Mirate, revisando historias como si no tuvieras cosas que hacer",
    "Hey, no podés pasar todo el día revisando historias de los demás",
    "Creo que tenes cosas que hacer, y vos acá",
    "Sí, muy chidas las historias de otros y toda la onda, pero tenés cosas que hacer",
    "Disculpá que te arruine el rato, pero tenés cosas que hacer, 'aquello' acordate",
    "Tanto que pasaste hablando de terminar tus ondas, solo sos paja",
    "¡Uy si! 'Tomando un descando', no cuenta si lo hacés cada 15 minutos",
    "Vergón, supongamos que ya no tenés nada que hacer, supongamos",
    "Chidas las historias, te comprendo, pero puya, terminá lo que tenés que hacer",
    "Teniendo la oportunidad de desocuparte rápido pero, perdés el tiempo",
    "El día que sin tanta vuelta hagás lo que tenés que hacer, ése día será bueno",
    "¿Sabías que cada vez que organizás tus tiempos se salva un perrito abandonado?",
    "Siempre decís que para mañana, pero, que casualidad, por lo visto nunca es 'mañana'",
    "Bien, es hora de ponernos serios, pensá, ¿Qué tenés que hacer hoy?",
    "Talvéz no vas a llegar a ser el mejor, pero tampoco seás de los peores",
    "No, no soy psicólogo, no necesito serlo para decirte que dejés de perder el tiempo",
    "¿De qué sirve que planiés todo si al final de cuentas nunca hacés las ondas?",
    "¿Sabías que aparte de ver fotos de comida por acá, podés pensar en las cosas pendientes?",
    "No tengo una frase por el momento, estoy descansando. ¡Pajas! Mejor andá terminá 'aquello'",
    "Podría decirte frases bonitas de motivación, pero lo siento, no soy de esa onda de Forex",
    "Mis frases de motivación son un chiste, pero no más que tu organización de tiempo"
]

TEXTS_PUBLICATION = {
    "photo": TEXT_PUBLICATION_PHOTO,
    "story": TEXT_PUBLICATION_STORY,
    "comment": TEXT_COMMENTS
}

QUOTES_PUBLICATION = {
    "photo": QUOTES_PHOTO,
    "story": QUOTES_STORY,
}
